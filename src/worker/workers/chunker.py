...
class UserChunkerWorker(BaseWorker):
    ...
    def chunk_group_to_users(self, group_id):
        offset = 0
        while True:
            result = self.db.get_users_from_group(
                group_id, config.notifications.chunk_size, offset
            )
    ...
    def handler(self, ch, method, properties, body):
        ...
        if rabbit_message.context.users_id:
            ...
            chunk_users = partial(
                chunked,
                rabbit_message.context.users_id,
                config.notifications.chunk_size,
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--realtime",
        action="store_true",
        help="send to realtime mailer or background queue for further enrich, default true",
    )
    ...
    parser.add_argument(
        "--background",
        action="store_true",
        help="send to background queue for further enrich, default false",
    )
